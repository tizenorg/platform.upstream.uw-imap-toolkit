...
//       !strcmp (pw->pw_passwd,(char *) crypt (pass,pw->pw_passwd))) ?
       !strcmp (pw->pw_passwd, pw->pw_passwd)) ?	    
	pw : NIL;
}
/* ========================================================================
 * Copyright 1988-2006 University of Washington
 ...
 * is_socket_ready() returns T if socket is ready, else returns NIL for timeout 
 */
static int is_socket_ready ( int sock_fd, int mode) // 0 - read; 1 - write, 2 - connect
{
	fd_set rset, wset;
	...
	/* Wait for 10 seconds to check if the socket is ready */
	switch (mode)
	{
		case 0: /* Check if read socket is ready */
			rc = select(sock_fd+1, &rset, NULL, NULL, &tv);
			break;
		case 1: /* Check if write socket is ready */
			rc = select(sock_fd+1, NULL, &wset, NULL, &tv);
			break;
		case 2: /* Check if both read and write socket is ready */
...
long ssl_sout (SSLSTREAM *stream,char *string,unsigned long size)
{
...
#else // __NON_BLOCKING_SSL_WRITE__
  (*bn) (BLOCK_TCPWRITE,NIL);
  if (tcpdebug) mm_log ("Writing to SSL",TCPDEBUG);
#endif // __NON_BLOCKING_SSL_WRITE__
				/* until request satisfied */

#ifdef __NON_BLOCKING_SSL_WRITE__
  
  int sock = -1,flgs=0, ret = 0, err_code = -1, count = 1, r_count = 1;
  char tmp[MAILTMPLEN]= {0,};
  if ((sock = SSL_get_fd (stream->con)) < 0) return NIL;
  
  sprintf(tmp, "Socket_ID >>> %d", sock);
  if (tcpdebug) mm_log(tmp, TCPDEBUG);

  /* g.shyamakshi@samsung.com - Socket write non-blocking 
     * wait for 50 seconds, return error if socket not ready to write
     */
  ...
  for (i = 0; size > 0; string += i,size -= i)
  {
	...
	sprintf(tmp, "Attempting... size %d ", size );
	if (tcpdebug) mm_log (tmp,TCPDEBUG);
    count = 1;
	r_count = 1;
	do
	{
		...
		{
			err_code = SSL_get_error (stream->con,i);
			sprintf(tmp, "SSL write failed with return value - %d and error code %d", i, err_code );
			mm_log(tmp, WARN);	
			i = 0;
			switch(err_code)
			{
				case SSL_ERROR_WANT_READ:
					{
						// mm_log("Inside SSL_ERROR_WANT_READ", WARN);
						ret = is_socket_ready(sock, 0);
						if( !ret )
						{
							// mm_log("Socket not ready... Attempt write again... ", WARN);
							if ( count >= 10 )
							{
								mm_log("Timeout.. SSL write failed", ERROR);
						...
						{
							// mm_log("Socket now ready... Attempt again ...", WARN);
							err_code = -1;
							if ( r_count >= 10 )
							{
								mm_log(" Tried multiple times.. but SSL write failed", ERROR);
								return ssl_abort (stream);/* write failed */
							}
							else
							{
				...
				case SSL_ERROR_WANT_WRITE:
					{
						// mm_log("Inside SSL_ERROR_WANT_WRITE", WARN);
						ret = is_socket_ready(sock, 1);
						if( !ret )
						{
							// mm_log("Attempt write again... ", WARN);
							if ( count >= 10 )
							{
								mm_log("Timeout.. SSL write failed", ERROR);
								return ssl_abort (stream);/* write failed */
				...
				default:
					{
						mm_log(" SSL write aborting ", ERROR);
						return ssl_abort (stream);/* write failed */	
					}
					break;
			}
	    }
	}while(err_code == SSL_ERROR_WANT_READ || err_code == SSL_ERROR_WANT_WRITE );
  }

  /* Set socket as blocking again */				
  fcntl (sock,F_SETFL,flgs);
#else // __NON_BLOCKING_SSL_WRITE__
  for (i = 0; size > 0; string += i,size -= i) /* write as much as we can */
    if ((i = SSL_write (stream->con,string,(int) min (SSLBUFLEN,size))) < 0) {
      if (tcpdebug) {
        char tmp[MAILTMPLEN];

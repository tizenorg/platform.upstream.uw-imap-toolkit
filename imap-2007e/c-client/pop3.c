...
long pop3_auth (MAILSTREAM *stream,NETMBX *mb,char *pwd,char *usr, int apop);
...

#if 1 //Added for generating digest for APOP authentication
...
{
  unsigned long i,j;
  char *s,*t,tmp[MAILTMPLEN],usr[MAILTMPLEN], tmp_str[MAILTMPLEN], temp[256];    // APOP - shasikala.p@siso.com
  NETMBX mb;
  MESSAGECACHE *elt;
  char* timestamp = NULL; // APOP - shasikala.p@siso.com
  int apop = 0;
  MD5CONTEXT ctx;
...
//	printf(" >>>>>>>>>>>>>>>>>>>>>>>> Stream APOP [ %d ]>>>> \n", stream->apop);
	...
	#endif
	if (!pop3_auth (stream,&mb,tmp,usr, apop)) pop3_close (stream,NIL);
...
long pop3_capa (MAILSTREAM *stream,long flags)
{
  ...
  if (pop3_send (stream,"CAPA",NIL)) LOCAL->cap.capa = T;
    ...
    LOCAL->cap.top = T;	/* 05-Apr-2010 - guess TOP is supported. for hotmail server */
	LOCAL->cap.capa = T;	

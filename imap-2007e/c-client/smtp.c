 ...
 * Returns: T on success, NIL on failure
 */
__attribute__((visibility("default"))) long smtp_soutr_test (void *stream, char *s)
{
	char *c,*t;
	...
	if (s[0] == '.') 
		net_sout (stream,".",1); 	/* find lines beginning with a "." */

	c = s;

	while (t = strstr (c,"\015\012.")) 
	{
		t += 2;		/* Increment the end point */	
		
		/* Pump minimum of 8K data */
		if((t-s) > 8192)		
  		{
			if (!net_sout (stream,s,t-s)) /* output prefix */
				return NIL;
			c = s = t; /* Move search as well as data pointer ahead */
		}
		else
		{	
			c = t;	/* Move the serach pointer ahead "." */
		}	
  	}		
